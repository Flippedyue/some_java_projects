            ...
            <div class="row clearfix">
                <div class="col-md-12 column">
                    <div class="page-header">
                        <c:choose>
                            <c:when test="${requestScope.get('status') == false}">
                                <h2>${requestScope.get('result')}</h2>
                            </c:when>
                            <c:otherwise>
                                <c:choose>
                                    <c:when test="${requestScope.get('flag') == false}">
                                        <h2>${requestScope.get('result')}</h2>
                                    </c:when>
                                    <c:otherwise>
                                        <h2>消费场景：${requestScope.get('description')}，
                                            实际使用了${requestScope.get('num')}${requestScope.get('unit')}</h2>
                                        <h3>${requestScope.get('result')}</h3>
                                    </c:otherwise>
                                </c:choose>
                            </c:otherwise>
                        </c:choose>
